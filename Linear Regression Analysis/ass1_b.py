...
def grad_desc(learn_rate,n_features,n_iterations,x_vectors,y_values,regularization_param=0,cost_func_type='MSE'):	
...
# for linear , the irls update is one-step and the number of iterations is not used at all: the...
# min. is obtained in a single step

def irls(learn_rate,n_features,n_iterations,x_vectors,y_values,cost_func_type='MSE'):
	phi_mat=x_vectors#.transpose()
	theta_vector=np.dot(phi_mat.transpose(),phi_mat)
	theta_vector=np.linalg.inv(theta_vector)
	theta_vector=np.dot(theta_vector,phi_mat.transpose())
	theta_vector=np.dot(theta_vector,y_values)
...
def main():
	...
	n_iterations=[0,20,50,80,100,200,300,500,1000,1100]#,1000,5000]#,100,200,500,1000]
	errors_GD=[]
	errors_irls=[]
	for iter_count in n_iterations:
		print "\n\t#iterations: ",iter_count
		learned_params=learned_params=grad_desc(0.05,n_features,iter_count,train_param,train_target)
		...
		learned_params=learned_params=irls(0.05,n_features,iter_count,train_param,train_target)
		test_predictions=get_predictions(learned_params,test_param)
		error=get_mse(test_predictions,test_target,use_RMSE=True)
	...
	l1,l2=plt.plot(n_iterations,errors_GD,n_iterations,errors_irls)
	plt.setp(l1,linewidth=1,color='r')
	plt.setp(l2,linewidth=2,color='g')
	plt.legend([l1,l2],['Gradient Desc.','IRLS'])
	plt.xlabel('#iterations')
	plt.ylabel('RMSE')
	plt.savefig("ass1_b.png")
	#plt.show()
